...
def test_camadas_nao_erram_e_taxa_e_estavel_entre_sementes():
    ...
    for semente in range(1, 6):
        ...
        assert m.false_positives == 0, f"semente {semente}:
{m.render()}"
        assert m.false_negatives == 0, f"semente {semente}:
{m.render()}"
...
- [ ] A taxa determinística medida foi **registrada** em várias sementes, não em uma, e o spec seção 2.3 foi atualizado com a medição
